...
export function RuleGroupBar({
  ...
  return (
    <div className="space-y-1 border-l border-[#2b3139] pl-1" style={{ marginLeft: depth * 8 }}>
      <div className="flex h-7 items-center gap-1">
        ...
        <Button type="button" variant="ghost" className="h-6 px-2 text-[10px]" onClick={() => addCondition(node.id)}>
          <Plus className="mr-1 h-3 w-3" />
          Cond
        </Button>
        <Button type="button" variant="ghost" className="h-6 px-2 text-[10px]" onClick={() => addGroup(node.id)}>
      ...
      <div className="space-y-1">
        {node.children.map((child) =>
          child.type === "GROUP" ? (
            ...
            <RuleConditionChip key={child.id} node={child} />
          ),
        )}
      </div>
    </div>
