...
describe("operand defaults", () => {
  it("creates market operands", () => {
    ...
    const volume24h = createMarketOperand("volume24h", "15m");
    expect(volume24h).toEqual({ kind: "TICKER_VOLUME" });
  });
  ...
  it("reads operand categories and market fields", () => {
    ...
    expect(getMarketField(createMarketOperand("volume24h", "15m"))).toBe("volume24h");
