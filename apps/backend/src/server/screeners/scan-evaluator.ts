...
async function resolveOperand(operand: Operand, ctx: ScanEvalContext) {
  switch (operand.kind) {
    ...
    case "TICKER_VOLUME":
      return { current: ctx.ticker?.volume24h ?? NaN };
    case "MARKET_FIELD": {
      const candles = await ctx.loadCandles(operand.timeframe);
      if (candles.length === 0) return { current: NaN };
      const last = candles[candles.length - 1];
...
export async function evaluateTickerPhase(
  ...
  const result = await evaluateFastPassFilter(tree.root, ctx);
  return !result.failedFast;
}

async function evaluateFastPassFilter(
  node: RuleNode,
  ctx: ScanEvalContext,
): Promise<{ failedFast: boolean }> {
  if (node.type === "CONDITION") {
    if (getConditionDataCost(node) === "ticker") {
      const snapshot = await evaluateCondition(node, ctx);
      return { failedFast: !snapshot.passed };
    }
    return { failedFast: false };
  }

  if (node.operator === "AND") {
    for (const child of node.children) {
      const { failedFast } = await evaluateFastPassFilter(child, ctx);
      if (failedFast) return { failedFast: true };
    }
    return { failedFast: false };
  }
